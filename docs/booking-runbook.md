...
**Worst-case emails for one booking**, now that verification is gone and reminders exist: confirmation (booker) + notification (you) + reminder (booker, if the call is still on) + cancellation (booker) + cancellation (you) = 5, same total as before this change (it swaps the old verification step for the new reminder step). At `maxCallsPerDay = 2`, that's at most 10 booking emails/day even in the worst case — comfortably under the 40/day default cap.
